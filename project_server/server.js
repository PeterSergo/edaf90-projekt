...
const { v4: uuidv4 } = require("uuid"); //ta bort i guess eller ändra till movie id typ
...
(function () {
  ...
  function getDetails(req, res, next, kind) {
    const obj = inventory[req.params.name]; //byta ut inventory?
    if (obj) {
      if (obj[kind]) {
        res.json(obj);
      } else {
